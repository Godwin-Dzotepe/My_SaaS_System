...
export default function PresentStudentsPage() {
  ...
  useEffect(() => {
    ...
    const fetchPresentStudents = async () => {
      ...
      setError(null);
      try {
        const res = await fetch(`/api/teacher/attendance/present?class_id=${selectedClassId}&date=${selectedDate}`);
        if (!res.ok) {
          throw new Error('Failed to fetch present students');
        }
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <CheckCircle2 className="w-6 h-6 text-green-500" /> Present Students
          </CardTitle>
          <CardDescription>View students marked as present for a specific class and date.</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          {selectedClassId && presentStudents.length === 0 && !loading && (
            <div className="p-4 bg-yellow-50 text-yellow-800 rounded-lg">
              No students marked as present for this class and date.
            </div>
          )}
          ...
          {selectedClassId && presentStudents.length > 0 && (
            <div className="space-y-4">
              <h3 className="text-lg font-semibold text-gray-900 flex items-center gap-2">
                <Users className="w-5 h-5" /> Present Students in {classes.find(c => c.id === selectedClassId)?.class_name} on {selectedDate}
              </h3>

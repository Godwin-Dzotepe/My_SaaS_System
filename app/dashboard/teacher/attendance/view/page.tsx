...
import { Calendar as CalendarIcon, Loader2, CheckCircle2, XCircle, Users } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { Card, CardContent, CardHeader, CardTitle, CardDescription } from '@/components/ui/card';
import { Input } from '@/components/ui/input'; // Assuming you have an Input component for date
...
export default function ViewAttendancePage() {
  ...
  const [attendanceSummary, setAttendanceSummary] = useState<StudentAttendance[]>([]);
  ...
  useEffect(() => {
    ...
    const fetchAttendanceSummary = async () => {
      ...
      setError(null);
      try {
        const res = await fetch(`/api/teacher/attendance/summary?class_id=${selectedClassId}&date=${selectedDate}`);
        if (!res.ok) {
  ...
  const presentCount = attendanceSummary.filter(sa => sa.status === 'present').length;
  const absentCount = attendanceSummary.filter(sa => sa.status === 'absent').length;
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <CalendarIcon className="w-6 h-6" /> View Daily Attendance
          </CardTitle>
        ...
        <CardContent className="space-y-4">
          ...
          {selectedClassId && attendanceSummary.length === 0 && !loading && (
                    ...
                    {attendanceSummary.map((student) => (
                      <tr key={student.student_id}>
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                          {student.student_name} {student.student_number && <Badge variant="outline" className="ml-2">{student.student_number}</Badge>}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                          <Badge variant={student.status === 'present' ? 'default' : 'destructive'}>
                            {student.status === 'present' ? <CheckCircle2 className="w-4 h-4 mr-1" /> : <XCircle className="w-4 h-4 mr-1" />}

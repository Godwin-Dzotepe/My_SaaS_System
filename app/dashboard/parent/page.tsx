          ...
          >
            ...
            <div className="flex items-center gap-2">
              ...
              <Button variant="outline" className="gap-2 border-[#ff9d01] text-[#ff9d01] hover:bg-[#ff9d01] hover:text-white transition-colors">
                ...
                <Badge className="bg-[#ff9d01] text-white border-none ml-1">{dashboard?.stats.notificationsCount ?? 0}</Badge>
              </Button>
            </div>
          </motion.div>
                ...
                <Card className="border-none shadow-sm overflow-hidden bg-white group hover:shadow-md transition-shadow">
                  <div className="flex items-center p-6 gap-4">
                    ...
                    <div>
                      ...
                      <h3 className="text-2xl font-bold text-[#212529]">{dashboard?.stats.totalChildren ?? 0}</h3>
                    </div>
                  </div>
                </Card>
                ...
                <Card className="border-none shadow-sm overflow-hidden bg-white group hover:shadow-md transition-shadow">
                  <div className="flex items-center p-6 gap-4">
                    ...
                    <div>
                      ...
                      <h3 className="text-2xl font-bold text-[#212529]">{dashboard?.stats.averageAttendance ?? 'N/A'}</h3>
                    </div>
                  </div>
                </Card>
                ...
                <Card className="border-none shadow-sm overflow-hidden bg-white group hover:shadow-md transition-shadow">
                  <div className="flex items-center p-6 gap-4">
                    ...
                    <div>
                      ...
                      <h3 className="text-2xl font-bold text-[#212529]">{dashboard?.stats.notificationsCount ?? 0}</h3>
                    </div>
                  </div>
                </Card>
              </motion.div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {dashboard?.children.length ? dashboard.children.map((child) => (
                  <motion.div key={child.id} variants={itemVariants}>
                    <Card className="border-none shadow-sm bg-white overflow-hidden hover:shadow-md transition-all group">
                      ...
                      <CardContent className="p-6">
                        ...
                        <div className="grid grid-cols-3 gap-2 py-5 bg-[#f8f9fb] rounded-xl px-4 mb-3">
                        ...
                        {child.latestPeriod ? (
                          <p className="text-xs text-[#646464] mb-6">Latest recorded term: {child.latestPeriod}</p>
                        ) : (
                  ...
                  <motion.div variants={itemVariants} className="lg:col-span-2">
                    <Card className="border-none shadow-sm bg-white">
                      <CardContent className="p-10 text-center text-[#646464]">
                  ...
                  <Card className="border-none shadow-sm bg-white h-full">
                    ...
                    <CardContent className="pt-6">
                      <div className="space-y-4">
                        {dashboard?.notifications.length ? dashboard.notifications.map((notification, idx) => (
                          <motion.div
                            key={notification.id}
                            initial={{ opacity: 0, x: -10 }}
                            animate={{ opacity: 1, x: 0 }}
                            transition={{ delay: 0.5 + idx * 0.1 }}
                            className="flex items-start gap-4 p-4 rounded-xl border border-gray-50 bg-[#f8f9fb] hover:bg-white hover:shadow-md hover:border-[#3f7afc]/20 transition-all group"
                          >
                          ...
                          <p className="text-center text-[#646464] py-8">No new notifications right now.</p>
                        )}
                      </div>
                    </CardContent>
                  </Card>

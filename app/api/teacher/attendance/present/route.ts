...
// GET /api/teacher/attendance/present
// Fetches students marked as present for a given class and date
export const GET = withAuth(async ({ session, req }) => {
  try {
      ...
      include: {
        ...
        attendance: {
          where: {
            ...
            status: 'present', // Only fetch present records
          },
          select: {
            student_id: true,
          },
    ...
    // Filter students who are marked as present
    const presentStudents = teacherClass.students.filter(student =>
      teacherClass.attendance.some(att => att.student_id === student.id)
    );

    return NextResponse.json(presentStudents);
